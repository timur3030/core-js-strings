...
function removeFirstOccurrences(str, value) {
  ...
  return str.slice(0, index).concat(str.slice(value.length + index));
}
...
function countVowels(str) {
  ...
  str.split('').forEach((element) => {
    if (vowels.includes(element)) result += 1;
  });
  return result;
}
...
function reverseWords(str) {
  ...
  function reverse(string) {
    ...
    for (let i = 0; i < string.length; i += 1) {
      array.push(string.charAt(string.length - 1 - i));
    }
    return array.join('');
  }
  str.split(' ').forEach((word) => {
    arr.push(reverse(word));
  });
  return arr.join(' ');
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((element) => {
      return abcRot13[abc.indexOf(element)];
    })
    .join('');
}
